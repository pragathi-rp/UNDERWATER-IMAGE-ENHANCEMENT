...
def get_img_lightness_hist(L, range_min=0, range_max=100, bins=50):
    ...
    hist, bin_edges = np.histogram(L.flatten(), bins, range=(range_min, range_max), density=False)
    hist = np.single(hist) / np.single(h * w)
    hist1, bin_edges1 = np.histogram(L1.flatten(), bins, range=(range_min, range_max), density=False)
    hist1 = np.single(hist1) / np.single(h * w)
    hist2, bin_edges2 = np.histogram(L2.flatten(), bins, range=(range_min, range_max), density=False)
